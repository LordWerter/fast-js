...
export default class WSConnect extends AWSConnect {
  ...
  constructor(settings: TSettings, eventCallbacks?: TEventCallbacks) {
    ...
    if (
      !settings &&
      !Object.keys.call(settings.host) &&
      this.settings.options.debug.checkConfig
    ) {
      console.error(`
   ...
   * @returns
   */
  private checkAndRunHandler = async (
    handlerName: string,
    eventObj: Event
  ...
  private handleOnClose = async (event: Event) => {
    const { enabled, timeout, increaseTimeout } = this.settings.reconnect;
    if (enabled)
      setTimeout(() => {
        this.socket = this.init();
    ...
    const onCloseResult =
      this.eventCallbacks && this.eventCallbacks.wasClosed
